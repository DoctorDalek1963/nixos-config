...
let
  ...
  ssh-add-commands = map (
    keyname:
    let
      script = pkgs.writeShellScriptBin "cat-passphrase" ''cat "${
        config.sops.secrets."ssh/${keyname}/passphrase".path
      }"'';
    in
    ''cat "${
      config.sops.secrets."ssh/${keyname}/keys/${keyname}".path
    }" | SSH_ASKPASS="${script}/bin/cat-passphrase" SSH_ASKPASS_REQUIRE=force ssh-add -''
  ) ssh-keys;

  populate-ssh-keychain-path =
    let
      script = pkgs.writeShellScriptBin "populate-ssh-keychain" (
        lib.strings.concatStringsSep "\n" ssh-add-commands
      );
    in
    "${script}/bin/populate-ssh-keychain";
in
{
  config = lib.mkIf keychain-enabled {
    ...
    programs =
      let
        ...
        shellCommand = "${cfg.package}/bin/keychain --eval ${lib.strings.concatStringsSep " " flags} ${lib.strings.concatStringsSep " " cfg.keys}";
      in
      {
        keychain = {
