...
in
{
  config = lib.mkIf cfgFf.enable {
    ...
    home.file = {
      ".librewolf/${cfg.username}/search.json.mozlz4".force = lib.mkForce true;

      ".librewolf/native-messaging-hosts/pywalfox.json" = lib.mkIf config.programs.noctalia-shell.enable {
