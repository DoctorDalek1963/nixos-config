...
in
{
  config = lib.mkIf (cfg.enable && cfgMs.enable && cfgMs.music) {
    ...
    systemd.services.navidrome.serviceConfig = {
      ...
      ExecStart = lib.mkForce "${pkgs.writeShellScript "start-navidrome.sh" ''
        ...
        ${config.services.navidrome.package}/bin/navidrome --configfile ${
          (pkgs.formats.json { }).generate "navidrome.json" config.services.navidrome.settings
        }
      ''}";
    };
  };
